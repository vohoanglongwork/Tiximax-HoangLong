...
test('Test tạo tài khoản purchase', async ({ page }) => {
  test.setTimeout(90000);
const randomUsername = `autopurchaser${Math.random().toString(36).substring(7)}`;
...
function generateRandomEmail() {
  ...
  return `test.purchaser_${timestamp}@gmail.com`;
}
...
await page.getByRole('button', { name: 'Nhân viên mua hàng' }).click();
...
await page.getByPlaceholder('Nguyễn Văn A').fill('Test Purchaser');
