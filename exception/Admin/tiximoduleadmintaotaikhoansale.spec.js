...
const permission = page.getByRole('generic', { name: 'Duyệt chi phí' });

await permission.getByRole('button').click();

const permission2 = page.getByRole('generic', { name: 'Yêu cầu chi phí' });

await permission2.getByRole('button').click();
  await expect(page.getByText('Nhân viên bán hàng')).toBeVisible({ timeout: 15000 });
});