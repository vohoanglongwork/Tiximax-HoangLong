...
await page.locator('input[type="file"]').setInputFiles('tests/fixtures/receipt.png');
 await expect(page.getByText('Tải ảnh lên thành công')).toBeVisible({ timeout: 5000 });
  ...
  const menu2 = page.getByRole('link', { name: 'Thêm mã vận đơn' });

await expect(menu2).toBeVisible();

await Promise.all([
  page.waitForURL('**/staff-purchaser/inventory'),
  menu2.click()
]);
await expect(menu2).toBeVisible();

await page.getByRole('button', { name: 'Thêm vận đơn' }).first().click();
await page.getByPlaceholder('Nhập hoặc quét mã vận đơn').fill(randomMVD);
