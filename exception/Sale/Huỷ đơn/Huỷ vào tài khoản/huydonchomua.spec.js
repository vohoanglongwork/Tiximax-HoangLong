...
const daXacNhanBtn = page.getByRole('button', { name: /Chờ mua/ });
...
await page.getByText('Hoàn vào ví').click({ force: true });
await page.getByText('Chuyển khoản ngân hàng').click();
await page.keyboard.press('Escape');
await page.getByText('Chọn hoặc nhập tên ngân hàng').click({ force: true });

await page.getByPlaceholder('Nhập số tài khoản').fill('01234566789');
await page.getByPlaceholder('Nhập tên chủ tài khoản').fill('Test');
const bankCombo = page.getByRole('combobox', {
  name: /Tên ngân hàng/i
});

await bankCombo.click();
await bankCombo.fill('Vietin');
