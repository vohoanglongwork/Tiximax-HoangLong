...
test('Test xác nhận thanh toán ship', async ({ page }) => {
...
const option = dropdown.getByText('Chờ thanh toán ship', { exact: true });
...
await option.click();
  ...
  await expect(
    page.getByRole('button', { name: 'Xác nhận ship', exact: true }).first()
  ).toBeVisible({ timeout: 15000 });


  await page.getByRole('button', { name: 'Xác nhận ship', exact: true }).first().click({ timeout: 5000 });
  ...
  await expect(page.getByText('Xác nhận thanh toán ship thành công cho đơn')).toBeVisible({ timeout: 15000 });
});